    ...
    if file and file.filename.endswith('.csv'):
        try:
            ...
            if new_units == 0 and new_classtimes == 0:
                flash("Upload skipped: All units and class times already exist.", "info")
            else:
                db.session.commit()
                flash(f"Upload complete: {new_units} new unit(s), {new_classtimes} new class time(s) added.", "success")
